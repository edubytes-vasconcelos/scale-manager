...
export default function Schedules() {
  ...
  const getServiceSummaryBadge = (service: Service) => {
    ...
    if (total === 0)
      return <Badge className="bg-slate-100 text-slate-700 border border-slate-300">Sem voluntários</Badge>;

    if (confirmed === total)
      return <Badge className="bg-green-100 text-green-800 border-2 border-green-300 font-semibold">✓ Completa</Badge>;
    ...
    return <Badge className="bg-slate-100 text-slate-700 border border-slate-300">Pendente</Badge>;
  };

  const getStatusBadge = (status?: string) => {
    if (status === "confirmed")
      return <Badge className="bg-green-100 text-green-800 border border-green-300 font-medium">✓ Confirmado</Badge>;
    if (status === "declined")
      return <Badge className="bg-red-100 text-red-800 border border-red-300 font-medium">✗ Recusou</Badge>;
    return <Badge className="bg-amber-100 text-amber-800 border border-amber-300">⏳ Pendente</Badge>;
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-4 flex-wrap pb-2">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1 ml-[52px]">Gerencie escalas e confirmações</p>
        </div>
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {isAdminOrLeader && (
            <Button onClick={() => setCreateDialogOpen(true)} className="transition-all hover:scale-[1.02] shadow-sm">
      ...
      {/* Filters Section */}
      <Card className="transition-all hover:shadow-md hover:scale-[1.01] mb-4">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            >
              <span className={quickChip === "myPending" ? "font-semibold" : ""}>
                Minhas pendentes
              </span>
              <Badge className={`ml-2 ${quickChip === "myPending" ? "bg-slate-100 text-slate-700" : "bg-amber-100 text-amber-800"}`}>
                {chipCounts.myPending}
              </Badge>
            </Button>
            ...
            <Button
              ...
              className="transition-all duration-200 hover:scale-105 shadow-sm"
            >
              <span className={quickChip === "today" ? "font-semibold" : ""}>
                Hoje
              </span>
              <Badge className={`ml-2 ${quickChip === "today" ? "bg-slate-100 text-slate-700" : "bg-slate-100 text-slate-700"}`}>
                {chipCounts.today}
              </Badge>
            </Button>
            ...
            <Button
              ...
              className="transition-all duration-200 hover:scale-105 shadow-sm"
            >
              <span className={quickChip === "week" ? "font-semibold" : ""}>
                Esta semana
              </span>
              <Badge className={`ml-2 ${quickChip === "week" ? "bg-slate-100 text-slate-700" : "bg-slate-100 text-slate-700"}`}>
                {chipCounts.week}
              </Badge>
            </Button>
            ...
            {hasActiveFilters && (
              <Button 
                ...
                className="ml-auto transition-colors hover:bg-muted hover:text-red-600"
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filteredServices.length === 0 ? (
            <div className="col-span-full text-center py-16">
              <div className="max-w-sm mx-auto">
                ...
                <p className="text-sm text-muted-foreground">
                  Ajuste os filtros ou crie uma nova escala
                </p>
                {hasActiveFilters && (
                  <Button 
                    variant="outline" 
            ...
            filteredServices.map((service) => {
              ...
              return (
                <Card key={service.id_uuid} className="transition-all hover:shadow-md hover:scale-[1.01] hover:scale-[1.01]">
                  <CardHeader className="pb-3 ">
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex-1">
                        ...
                        <div className="mt-3">{getServiceSummaryBadge(service)}</div>
                      </div>
                      ...
                      <div className="flex items-center gap-1">
                        {isAdminOrLeader && (
                          <Button
                            ...
                            className="transition-all hover:text-primary hover:bg-primary/10 hover:scale-110"
                          >
                        ...
                        {canDeleteService(service) && (
                          <Button
                            size="icon"
                            variant="ghost"
                            className="text-destructive transition-colors hover:bg-muted"
                  ...
                  <CardContent className="space-y-3">
                      ...
                      <div className="space-y-2 text-sm">
                        {assignments.map((a, idx) => (
                          <div
                            ...
                            className="flex items-center justify-between gap-2 p-2 rounded-md bg-slate-50 hover:bg-muted transition-colors"
                          >
                            <span className="truncate font-medium">{getVolunteerName(a.volunteerId)}</span>
                    ...
                    {my && !isPastService(service) && my.status === "pending" && (
                      <div className="flex gap-2 pt-3 border-t">
                        <Button 
                          size="sm" 
                          onClick={() => handleRSVPConfirm(service)} 
                          className="transition-all hover:scale-105 flex-1 shadow-sm"
                        >
                          <Check className="w-4 h-4 mr-1" /> Confirmar presença
                        </Button>
                        ...
                        <Button 
                          ...
                          className="transition-all hover:scale-105 hover:border-red-300 hover:text-red-600"
                        >
                    ...
                    {my?.status === "declined" && (
                      <div className="bg-red-50 border-2 border-red-200 rounded-lg p-3">
                        <Badge className="bg-red-100 text-red-800 border border-red-300 font-medium mb-1">
                          ✗ Você recusou esta escala
                        </Badge>
                        {(my as any).note && (
                          <p className="text-xs text-red-700 mt-2">
                            Motivo: {(my as any).note}
                          </p>
                        )}
                      </div>
                    )}
        ...
        <div className="border rounded-xl overflow-hidden shadow-sm">
          ...
          <div className="grid grid-cols-7">
            {calendarDays.map((day, idx) => (
              <div
                key={idx}
                className={`border p-2 min-h-[100px] transition-colors hover:bg-muted ${
                  !isSameMonth(day, calendarMonth) ? "bg-slate-50" : ""
                }`}
              >
                <div className="text-xs text-right font-medium">{format(day, "d")}</div>

                {servicesForDay(day).map((s) => (
                  <div
                    key={s.id_uuid}
                    className="text-xs bg-primary/10 rounded p-1 mt-1 truncate transition-colors hover:bg-primary/20 cursor-pointer"
      ...
      <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="text-base font-semibold">Nova Escala</DialogTitle>
            <DialogDescription className="text-sm text-muted-foreground">
              Crie uma nova escala para sua organização
            </DialogDescription>
          </DialogHeader>
          ...
          <DialogFooter>
            ...
            <Button onClick={handleCreateService} disabled={isSavingCreate} className="transition-colors">
              {isSavingCreate ? "Criando..." : "Criar"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={assignDialogOpen} onOpenChange={setAssignDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="text-base font-semibold">Gerenciar voluntários</DialogTitle>
            <DialogDescription className="text-sm text-muted-foreground">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <Label className="text-sm font-medium">Voluntários escalados</Label>
              ...
              {(selectedService?.assignments || []).length > 0 ? (
                <div className="space-y-2">
                  {(selectedService?.assignments || []).map((a, idx) => (
                    <div
                      key={`sel-${idx}`}
                      className="flex items-center justify-between gap-2 rounded-md border p-2 transition-colors hover:bg-muted"
                    >
            ...
            {/* ADICIONAR */}
            <div className="space-y-2 pt-2 border-t">
              <Label className="text-sm font-medium">Adicionar voluntário</Label>
              <Select
              ...
              >
                {isSavingAssign ? "Adicionando..." : "Adicionar"}
              </Button>
            </div>
          </div>
        </DialogContent>
      ...
      <Dialog open={declineDialogOpen} onOpenChange={setDeclineDialogOpen}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle className="text-base font-semibold">Motivo da recusa</DialogTitle>
            <DialogDescription className="text-sm text-muted-foreground">
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setDeclineDialogOpen(false)} className="transition-colors">
              Cancelar
            </Button>
            <Button
              variant="destructive"
