-- Allow optional email in onboarding flows (WhatsApp login support)
...
BEGIN
  ...
  v_phone := nullif(auth.jwt() -> 'user_metadata' ->> 'phone', '');
  v_user_name := COALESCE(auth.jwt() -> 'user_metadata' ->> 'name', v_email, v_phone, 'Usuário');
  ...
  ELSE
    INSERT INTO volunteers (id, auth_user_id, organization_id, access_level, name, email, whatsapp, created_at)
    VALUES (gen_random_uuid(), auth.uid(), v_org_id, 'volunteer', v_user_name, v_email, v_phone, now());
  END IF;
...
BEGIN
  ...
  ELSE
    ...
    INSERT INTO volunteers (id, auth_user_id, organization_id, access_level, name, email, whatsapp, created_at)
    VALUES (v_volunteer_id, auth.uid(), v_org_id, 'admin', v_user_name, v_email, v_phone, now());
  END IF;
